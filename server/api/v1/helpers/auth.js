...
exports.newToken = (user) => {
    return jwt.sign({ id: user.id, varifiedUser: user.verifiedUser },
        JWTSECRET, {
            expiresIn: JWTEXP,
        },
    );
...
exports.login = async(req, res) => {
    ...
    if (!password) {
        return res.status(400).send({ message: "password needed" });
    }
    if (!username && !email) {
        return res
            .status(400)
            .send({ message: "Either Email or Password is needed" });
    }
